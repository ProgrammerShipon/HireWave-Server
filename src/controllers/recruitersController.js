...
const deleteRecruiter = async (req, res) => {
    try {
        const id = req.params.id;
        const query = { _id: new ObjectId(id) }
        const deleteRecruiter = await recruitersCollection.findOneAndDelete(query)
        res.status(200).send(deleteRecruiter)
    } catch (error) {
        res.status(404).send({ message: error.message })
    }
}
module.exports = { getAllRecruiters, getRecruiter, postNewRecruiter, deleteRecruiter ,postNewRecruiters }